<template>
  <div id="danmaku" :class="[{ full: controls.full }, { smallScreen }, { tinyScreen }]" :style="danmakuStyle">
    <!-- 主容器，全屏作用的对象 -->
    <main class="danmaku-main">
      <!-- 视频 -->
      <video class="danmaku-video" :src="src"></video>
      <!-- 容器：绘制弹幕 -->
      <div @click="playOrPause" :class="['danmaku-danmu-container'].concat(setting.block)" id="danmaku-danmu-container"
        v-if="setting.onOff">
        <div v-for="danmu in list" :id="danmu.id" :key="danmu.id" :class="['danmaku-danmu', danmu.type]" :style="'color:' + danmu.color +
          ...
          ';visibility: show'">{{ danmu.content }}</div>
      </div>
      <div @click="playOrPause" :class="['danmaku-danmu-container'].concat(setting.block)" id="danmaku-danmu-container"
        v-else></div>
      <!-- 控件 -->
      <div class="danmaku-controls-container">
        <div class="dammaku-controls" @mousemove="controls.show = 3">
          <!-- 进度条控件 -->
          <div class="danmaku-progress-container">
            <div class="danmaku-progress">
              ...
              ]" v-model="controls.dragTime" :max="controls.durationTime" :format-tooltip="formatDragTime"
          ...
          <div class="danmaku-float-left">
            <!-- 播放/暂停按钮控件 -->
            <button class="danmaku-button" @click="playOrPause">
          ...
          <!-- 弹幕开关控件 -->
          <el-switch class="danmaku-switch" v-model="setting.onOff"></el-switch>
          <!-- 弹幕选项控件 -->
          <span class="danmaku-tooltip danmaku-setting">
            ...
            <div class="danmaku-tooltip-content">
              ...
              <div class="danmaku-line">
                ...
                <el-slider class="danmaku-line-slider" :show-tooltip="false" v-model="setting.opacity"></el-slider>
              </div>
              <!-- 弹幕选项-区域控件 -->
              <div class="danmaku-line danmaku-line2">
                <span class="danmaku-line-label">显示区域</span>
                <el-slider class="danmaku-line-slider" :max="3" :show-stops="true" :show-tooltip="false"
                  :marks="setting.marks" v-model="setting.zone"></el-slider>
              </div>
              <!-- 弹幕选项-速度控件 -->
              <div class="danmaku-line">
                <span class="danmaku-line-label">弹幕速度</span>
                <el-slider class="danmaku-line-slider" :min="1" :max="100" :show-tooltip="false"
                  v-model="setting.speed"></el-slider>
              </div>
              <!-- 弹幕选项-字体控件 -->
              <div class="danmaku-line">
                <span class="danmaku-line-label">字体大小</span>
                <el-slider class="danmaku-line-slider" :min="12" :max="48" :show-tooltip="false"
                  v-model="setting.size"></el-slider>
              </div>
            </div>
          </span>
          <!-- 弹幕输入栏控件 -->
          <el-input v-model="input" class="danmaku-input" :maxlength="50" placeholder="发个弹幕~"
            @keyup.enter.native="send">
            <!-- 弹幕输入栏选项控件 -->
            <span class="danmaku-tooltip danmaku-input-setting" slot="prepend">
              <i class="el-icon-s-open"></i>
              <div class="danmaku-tooltip-content">
                <!-- 弹幕输入栏选项-类型控件 -->
                <div>类型</div>
                <div class="danmaku-input-setting-block">
                  <button :class="['danmaku-block-button', { active: type === 'scroll' }]" @click="sendType('scroll')">
                    <svg class="icon" aria-hidden="true">
                      <use xlink:href="#icon-bofangqi-danmugundongkai"></use>
                    </svg>
                    <br>
                  ...
                  <button :class="['danmaku-block-button', { active: type === 'top' }]" @click="sendType('top')">
                    <svg class="icon" aria-hidden="true">
                      <use xlink:href="#icon-bofangqi-danmudingbukai"></use>
                    </svg>
                    <br>
                  ...
                  <button :class="['danmaku-block-button', { active: type === 'bottom' }]" @click="sendType('bottom')">
                    <svg class="icon" aria-hidden="true">
                      <use xlink:href="#icon-bofangqi-danmudibukai"></use>
                    </svg>
                    <br>
            ...
            <!-- 弹幕输入栏-发送按钮控件-->
            <el-button class="danmaku-input-send" @click="send" slot="append">发送</el-button>
          </el-input>
          <!-- 信息控件 -->
          <span class="danmaku-info" v-if="!hideInfo">
          ...
          <div class="danmaku-float-right">
            ...
            <span class="danmaku-tooltip danmaku-rate">
              <div class="danmaku-tooltip-content">
                <el-slider v-model="controls.rate" class="volumeSlider" :min="1" :max="12" vertical
                  :show-tooltip="false" height="130px"></el-slider>
              </div>
...
export default {
  ...
  methods: {
    ...
    send() {
      ...
      console.log('this.video', this.video.currentTime);
...
.danmaku-controls-container {
  ...
  & .dammaku-controls {
    ...
    & .danmaku-input {
      ...
      margin-top: 2px;
      border-radius: 4px;
      background: linear-gradient(rgba(rgb(80, 110, 192), 0.5),
